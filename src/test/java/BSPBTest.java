...
public class BSPBTest extends BaseTest {
    ...
    public void testBSPB_Successfully() {
        ...
        new AuthPage(driver).authenticate("demo", "demo");
        //Отображается форма двухфакторной аутентификации
        Assert.assertFalse(driver.findElements(By.id("otp-code-text")).isEmpty());
        //Ввести в поле ввода кода подтверждения 0000. Нажать на кнопку войти
        new VerifyBySmsPage(driver).confirmByCode("0000");
        ...
        By financeFreedom = mainPage.getFinanceFreedom();
        //На странице отображается блок «Финансовая свобода»
        Assert.assertTrue(driver.findElement(financeFreedom).isDisplayed(), "Блок «Финансовая свобода» не отображается");
